...
public sealed class AudioManager : MonoBehaviour
{
    ...
    public AudioClip EndingGameOverClip;
    [Header("初始静音")]
    ...
    private static AudioSource _backgroundAudio;
    ...
    public static AudioManager getInstance { get { return Instance; } }
    ...
    public static void PlayBackgroundMusic(AudioClip clip, bool isLoop = true, float speed = 1)
    {
        ...
        Debug.Log("Play Bgm "+ _backgroundAudio.clip.name);
    ...
    /// <param name="isLoop">是否循环</param>
    /// <param name="speed">播放速度</param>
    public void PlaySingleSound(AudioClip clip, float volume = 1f, bool isLoop = false, float speed = 1)
    {
        if (_singleAudio.isPlaying)
        {
            _singleAudio.Stop();
    ...
    /// <param name="isLoop">是否循环</param>
    /// <param name="speed">播放速度</param>
    public void PlayWorldSound(GameObject attachTarget, AudioClip clip, float volume = 1,bool isLoop = false, float speed = 1)
    {
        ...
        {
            AudioSource audio = AttachAudioSource(attachTarget, WorldPriority, WorldVolume*volume, 1, 1);
    ...
    /// <param name="attachTarget">附加目标</param>
    /// <param name="isGradual">是否渐进式</param>
    public void PauseWorldSound(GameObject attachTarget, float gradualTime)
    {
        if (_worldAudios.ContainsKey(attachTarget))
        {
            AudioSource audio = _worldAudios[attachTarget];
            if (gradualTime!=0)
            {
                audio.DOFade(0, gradualTime).OnComplete(() =>
                {
    ...
    /// <param name="attachTarget">附加目标</param>
    /// <param name="isGradual">是否渐进式</param>
    public void StopWorldSound(GameObject attachTarget, float gradualTime)
    {
        if (_worldAudios.ContainsKey(attachTarget))
        {
            AudioSource audio = _worldAudios[attachTarget];
            ...
            _worldAudios.Remove(attachTarget);
        }
    }
    /// <summary>
    /// 恢复播放指定的世界音效
    ...
    // {
    //     PlaySingleSound(EndingGameOverClip);
    // }
    // [YarnCommand("playBgmDis")]
    // public void PlayBgmDis()
    // {
    //     PlayBackgroundMusic(bgmList[1],false);
    // }
}